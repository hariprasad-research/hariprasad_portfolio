      ...
      <main className="pt-24 pb-20">
        <div className="container mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => <Card key={index} className="shadow-card hover:shadow-elegant transition-all duration-300 hover:-translate-y-1 cursor-pointer" onClick={() => setSelectedProject(project)}>
                <CardHeader>
        ...
        <DialogContent className="max-w-lg data-[state=open]:animate-[scale-in_0.3s_ease-out,fade-in_0.2s_ease-out] data-[state=closed]:animate-[scale-out_0.2s_ease-out,fade-out_0.2s_ease-out]">
          <DialogHeader>
            <div className="flex items-center gap-2 mb-2">
              <Badge variant="outline" className="text-xs animate-fade-in" style={{
              animationDelay: '0.1s'
            }}>
                {selectedProject?.type}
              </Badge>
              <Badge className={`text-xs animate-fade-in ${selectedProject ? getStatusColor(selectedProject.status) : ''}`} style={{
              animationDelay: '0.15s'
            }}>
                {selectedProject?.status}
              </Badge>
            </div>
            <DialogTitle className="text-xl animate-fade-in" style={{
            animationDelay: '0.2s'
          }}>{selectedProject?.title}</DialogTitle>
            <DialogDescription className="text-muted-foreground pt-2 animate-fade-in" style={{
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {selectedProject?.tags.map((tag, index) => <Badge key={index} variant="secondary" className="animate-fade-in" style={{
